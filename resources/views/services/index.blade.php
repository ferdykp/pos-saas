<x-app-layout>
    ...
    <div class="px-4 py-6 mx-auto md:px-6 lg:px-8 max-w-desktop">
        <div class="flex flex-col justify-between gap-4 pb-6 mb-6 border-b md:flex-row md:items-center border-border-200">
            <div>
                <div class="flex items-center gap-2 mb-2 text-[10px] font-bold tracking-widest uppercase text-primary-600"><i class="fa-solid fa-screwdriver-wrench"></i> Layanan Pelanggan</div>
            ...
            <a href="{{ route('pos.index') }}" class="inline-flex items-center justify-center h-10 gap-2 px-4 text-xs font-semibold text-white rounded-md shadow-sm bg-primary-600 hover:bg-primary-700"><i class="fa-solid fa-plus"></i> Transaksi Baru</a>
        </div>

        @if(session('success'))<div role="status" class="flex items-center gap-2 p-4 mb-5 text-sm border rounded-lg bg-primary-50 border-primary-100 text-primary-700"><i class="fa-solid fa-circle-check"></i>{{ session('success') }}</div>@endif
                ...
                <article class="overflow-hidden border rounded-lg shadow-sm bg-surface-0 border-border-200">
                    <div class="flex items-start justify-between gap-3 p-5 border-b bg-surface-100/60 border-border-200">
                        <div class="min-w-0">
                            ...
                            <p class="mt-1 text-[10px] text-ink-400"><i class="mr-1 fa-regular fa-calendar"></i>{{ $order->created_at->format('d M Y · H:i') }}</p>
                        </div>
                    ...
                    <div class="p-5">
                        <div class="flex items-center gap-2 mb-4"><span class="flex items-center justify-center w-8 h-8 rounded-full bg-primary-50 text-primary-600"><i class="text-xs fa-solid fa-user"></i></span><div><p class="text-[10px] uppercase tracking-wider text-ink-400 font-bold">Pelanggan</p><p class="text-xs font-semibold text-ink-900">{{ $order->customer?->name ?? 'Pelanggan umum' }}</p></div></div>
                        ...
                        @if($overdue)<div class="flex items-center gap-2 p-2.5 mb-4 text-[11px] font-semibold rounded-md bg-red-50 text-semantic-danger"><i class="fa-solid fa-triangle-exclamation"></i>Melewati estimasi selesai</div>@endif
                        ...
                        <form action="{{ route('services.update', $order) }}" method="post" class="space-y-3">
                            ...
                            <button class="inline-flex items-center justify-center w-full h-10 gap-2 text-xs font-semibold text-white rounded-md bg-primary-600 hover:bg-primary-700"><i class="fa-solid fa-floppy-disk"></i>Simpan Pengerjaan</button>
                        </form>
                    </div>
                </article>
            @empty
                <div class="py-16 text-center border border-dashed rounded-lg md:col-span-2 xl:col-span-3 bg-surface-0 border-border-200"><div class="flex items-center justify-center w-12 h-12 mx-auto mb-3 rounded-full bg-surface-100 text-ink-400"><i class="fa-solid {{ $icons[$status] }}"></i></div><p class="text-sm font-semibold text-ink-900">Belum ada pekerjaan {{ strtolower($labels[$status]) }}</p><p class="mt-1 text-xs text-ink-400">Order jasa dengan status ini akan muncul di sini.</p></div>
            @endforelse
        </div>
